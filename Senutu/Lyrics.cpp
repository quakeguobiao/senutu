...
QString Lyrics::At( qint64 timePoint )
{
	QMap<qint64,QString>::iterator it = _lrcData.lowerBound(timePoint);
	while (it!=_lrcData.begin() && it.key()>timePoint)
		it--;
	if (it.key()>timePoint)
		return "...";
	if (it!=_lrcData.end()) {
...
void Lyrics::prepareLrc()
{
	...
	for (int i = 0;i<m_temp.size();i++)
		if (m_temp[i]=='[') {
			int posMaohao = m_temp.indexOf(':',i);
			if (posMaohao==-1/*std::string::npos*/)
				continue;			
			QString minuteStr = m_temp.mid(i+1,posMaohao-i-1);
			bool ok;
			int minute = minuteStr.toInt(&ok);
			if (!ok)
				continue;
			int posFang = m_temp.indexOf(']',i);
			if (posFang==-1/*std::string::npos*/)
				continue;
			QString secStr = m_temp.mid(posMaohao+1,posFang-posMaohao-1);
			double second = secStr.toDouble(&ok);
			if (!ok)
				continue;
			int nextFang = m_temp.indexOf('[',i+1);			
			if (nextFang==-1) {
				nextFang = 9999999;
			}
